...
exports.setModuleLocalPath = setModuleLocalPath;
exports.setModuleFavorite = setModuleFavorite;
...
function getRecentModules(limit = 20) {
  ...
  const stmt = db.prepare(`
    SELECT m.id, m.artist_id, m.file_name, m.module_name, m.md5, m.local_path, COALESCE(m.favorite,0) as favorite, a.name as artist_name
    FROM modules m
...
function setModuleLocalPath(moduleId, localPath) {
  if (useMock) {
...
function setModuleFavorite(moduleId, favorite) {
  if (useMock) {
    const m = MOCK_MODULES.find((m) => m.id === moduleId);
...
function getFavorites() {
  ...
  const stmt = db.prepare(
    "SELECT m.id, m.artist_id, m.file_name, m.module_name, m.md5, m.local_path, COALESCE(m.favorite,0) as favorite, a.name as artist_name FROM modules m JOIN artists a ON m.artist_id = a.id WHERE m.favorite = 1 ORDER BY a.name, m.module_name",
  );
  const rows = [];
  while (stmt.step()) rows.push(stmt.getAsObject());
  return rows.map((r) => ({
...
function getDownloadedModules() {
  ...
  const stmt = db.prepare(
    "SELECT m.id, m.artist_id, m.file_name, m.module_name, m.md5, m.local_path, COALESCE(m.favorite,0) as favorite, a.name as artist_name FROM modules m JOIN artists a ON m.artist_id = a.id WHERE m.local_path IS NOT NULL ORDER BY a.name, m.module_name",
  );
  const rows = [];
  while (stmt.step()) rows.push(stmt.getAsObject());
  return rows.map((r) => ({
